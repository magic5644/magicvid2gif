...
`, '');
    ...
    const execFileStub = sinon.stub().callsFake((...args: any[]) => {
      ...
      const cb = args.at(-1);
      if (cmd === '/usr/bin/ffmpeg' && cmdArgs[0] === '-version') {
    ...
    sinon.assert.calledWith(execStub, 'command -v ffmpeg');
    sinon.assert.calledWith(execFileStub, '/usr/bin/ffmpeg', ['-version']);
  });
  ...
  test('detect_system_ffmpeg_returns_absolute_path_on_windows', async () => {
    ...
    const execFileStub = sinon.stub().callsFake((...args: any[]) => {
      ...
      const cmdArgs = args[1];
      const cb = args.at(-1);
      if (cmd === 'where' && cmdArgs[0] === 'ffmpeg') {
        return cb(null, String.raw`C:\ffmpeg\bin\ffmpeg.exe
`, '');
      }
      if (cmd === String.raw`C:\ffmpeg\bin\ffmpeg.exe` && cmdArgs[0] === '-version') {
        return cb(null, 'ffmpeg version 4.3.2', '');
      }
      return cb(new Error('unexpected'));
    });
    ...
    sinon.assert.calledWith(execFileStub, 'where', ['ffmpeg']);
    sinon.assert.calledWith(execFileStub, String.raw`C:\ffmpeg\bin\ffmpeg.exe`, ['-version']);
  });
  ...
  test('resolveBinary_supports_windows_linux_and_macos_x64_and_arm64', () => {
    const module = proxyquire('../../ffmpegManager', {});
    module.FfmpegManager.instance = undefined;
    const deps = makeDeps();
    const mgr = module.FfmpegManager.getInstance(deps) as unknown as {
      resolveBinary: (platform: NodeJS.Platform, arch: string) => { label: string; primary: { executableName: string; extraExecutables?: string[] } } | null;
    };

    const supportedTargets: Array<[NodeJS.Platform, string]> = [
      ['win32', 'x64'],
      ['win32', 'arm64'],
      ['linux', 'x64'],
      ['linux', 'arm64'],
    ...
    for (const [platform, arch] of supportedTargets) {
      const binary = mgr.resolveBinary(platform, arch);
      assert.ok(binary, `${platform}-${arch} should be supported`);
      assert.ok(binary.primary.executableName.startsWith('ffmpeg'));
    }

    assert.strictEqual(mgr.resolveBinary('linux', 'arm'), null);
    assert.strictEqual(mgr.resolveBinary('win32', 'ia32'), null);
  });
